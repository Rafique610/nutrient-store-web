  ...
  const handleAddproduct = async (e) => {
    ...
    try {
      const data = await adminApi.createproduct({
        ...
        description: newproduct.description || `${newproduct.title} is available on NutriFactor.`,
        developer: newproduct.developer || 'NutriFactor Wellness Lab',
      ...
      <div className="ag-table-wrap panel">
        {loading ? <div className="loading-screen"><div className="loader" /></div> : (
          <table className="ag-table"><thead><tr><th>Product</th><th>Health Goal</th><th>Price</th><th>Rating</th><th>Orders</th><th>Status</th><th className="text-right">Actions</th></tr></thead>
            <tbody>{filtered.map(product => { const colors = GENRE_COLORS[product.genre] || ['#4da6ff','#1a6dcc']; return (
