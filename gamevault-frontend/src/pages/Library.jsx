    ...
    <div className="library-page">
      <div className="library-header">
        <div>
          ...
          <p className="text-muted">{ownedGames.length} product{ownedGames.length !== 1 ? 's' : ''} in your order history</p>
        </div>
        {ownedGames.length > 0 && (
              ...
              <div key={game.id} className="lib-card panel">
                <div className="lib-card-cover" style={{ background: `linear-gradient(135deg, ${colors[0]}, ${colors[1]})` }}>
                  ...
                  <div className="lib-card-overlay">
                    <Link to={`/game/${game.id}`} className="lib-play-btn"><Icon name="visibility" size={16} /> View Details</Link>
                  </div>
                </div>
